...
async function enviarMensagemDoUsuario() {
  ...
  if (!cpf || !nomePaciente || !idadePaciente || !sexoPaciente || !dadosAnamnese) {
    await mostrarAlertaPersonalizado('Erro!', 'Por favor, preencha todos os campos antes de enviar.', 'erro');
  ...
  try {
    ...
    document.getElementById(loadingId)?.remove();
    await mostrarAlertaPersonalizado('Erro!', 'Erro ao processar a solicitação. Por favor, tente novamente mais tarde.', 'erro');
  } finally {
